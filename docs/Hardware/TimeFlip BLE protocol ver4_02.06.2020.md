...
Implementation note: observed firmware can emit ASCII status messages through the TimeFlip events characteristic during command writes. These are useful diagnostics when command-result output is stale or unclear, but they are not a substitute for the command acknowledgement contract and they may not include enough data for readback APIs.

- Changing the device name with `0x15 0x04 0x74 0x65 0x73 0x74` (`write name test`) changed the advertised name and emitted `0x4E656D6520736574`, ASCII text `Neme set`. This appears to be a firmware status message, probably intended as `Name set`.
- Writing pause-on with `0x06 0x01` emitted `0x7061757365204F4E`, ASCII text `pause ON`, without physical interaction with the device.
- Writing LED blink period emitted `0x73657420626C696E6B696E6720706572696F64`, ASCII text `set blinking period`.
- Writing LED brightness emitted `0x736574206272696768746E657373204C454473`, ASCII text `set brightness LEDs`.

Although TimeFlip event text is documented here for v4, the pause and LED status notifications have also been observed on v3 hardware. The LED messages confirm that a setting command was processed, but they do not include the current brightness or blink-period values.
